...
<section id="main">
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if(empty($username_err) && empty($password_err)){
            ...
            $sql = "SELECT artiste_name, password FROM members WHERE artiste_name = ?";
            ...
            if($stmt = mysqli_prepare($dbc, $sql)){
                ...
                if(mysqli_stmt_execute($stmt)){
                    ...
                    // Check if artiste_name exists, if yes then verify password
                    if(mysqli_stmt_num_rows($stmt) == 1){
                        ...
                        // Display an error message if artiste_name doesn't exist
                        $username_err = 'No account found with that artiste_name.';
                    }
                } else{
    ...
    <div class="wrapper">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
            ...
            <p>Don't have an account? <a href="membership.php">Sign up now</a>.</p>
        </form>
    </div>
</section>
